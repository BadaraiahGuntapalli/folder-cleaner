"""
Interactive right-click launcher for the Downloads Cleaner.
...
def menu_dupes(folder: str) -> None:
    ...
    console.print()
    run(folder, "dupes", "--dry-run", *(["--recursive"] if recurse else []), *(["--v"] if verbose else []))
    console.print()
    ...
    while True:
        ...
        console.print(
            Panel(
                ...
                title="[bold]Downloads Cleaner[/bold]",
                box=box.ROUNDED,
                padding=(1, 2),
            )
        )
