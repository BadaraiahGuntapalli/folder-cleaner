...
Examples:
  python main.py scan
  python main.py --folder "C:/Users/me/Downloads" scan --recursive
  ...
  python main.py clean -t all -f all --recursive --dry-run -v

  python main.py dupes --dry-run -v
  python main.py dupes --recursive

  python main.py config --show
  python main.py config --add-ext docx --add-file "keep.pdf" --add-folder "Work"
...
SCRIPT_DIR        = Path(__file__).parent.parent   # project root — stores config.json & cleaner.log
...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    console.print("[dim]Tip: [bold]python main.py clean -t ext1,ext2 --dry-run -v[/bold]  to preview deletion.[/dim]")
    if LOG_FILE.exists():
...
def cmd_clean(args: argparse.Namespace) -> None:
    ...
    if args.types:        lines += f"  Types     : [cyan]{args.types}[/cyan]\n"
    if args.older_than:   lines += f"  Age filter: older than [cyan]{args.older_than}[/cyan] days\n"
    if args.larger_than:  lines += f"  Size filter: larger than [cyan]{args.larger_than}[/cyan]\n"
    if args.recursive:    lines += f"  Scan      : [cyan]recursive[/cyan]\n"
    if skipped_count:     lines += f"  Skipped   : [yellow]{skipped_count}[/yellow] file(s) (excluded/filtered)\n"
    if matched_files:     lines += f"  Files     : [magenta]{len(matched_files)}[/magenta]  ({format_size(total_file_size)})\n"
    if matched_folders:   lines += f"  Folders   : [yellow]{len(matched_folders)}[/yellow]  ({format_size(total_folder_size)})\n"
    lines += f"  Total     : [bold green]{format_size(total_size)}[/bold green]"

    console.print(Panel(lines, title="Folder Cleaner", box=box.ROUNDED))
    ...
    if args.dry_run:
        ...
        console.print(
            ...
            f"  [dim]python main.py clean{hints}[/dim]\n"
        )
        return
...
def cmd_dupes(args: argparse.Namespace) -> None:
    ...
    for idx, (_, group) in enumerate(sorted(dupe_groups.items()), 1):
        group_sorted = sorted(group, key=lambda f: f.stat().st_mtime)
        keep   = group_sorted[0]
...
def cmd_config(args: argparse.Namespace) -> None:
    ...
    if args.show:
        ...
        for name in cfg["excluded_files"]:      table.add_row("file",      name)
        for ext  in cfg["excluded_extensions"]: table.add_row("extension", f".{ext}")
        for d    in cfg["excluded_folders"]:    table.add_row("folder",    d)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cleaner",
        description="Folder cleaner — scan, filter, deduplicate, and delete any folder.",
...
examples:
  ...
  python main.py dupes --dry-run -v
  python main.py dupes --recursive -v

  python main.py config --show
  python main.py config --add-ext docx --add-ext xlsx
  python main.py config --add-file "important.pdf" --add-folder "Work"
  python main.py config --remove-ext docx
        """,
    )
    ...
    sp = subs.add_parser("scan", help="Show all file types and folders")
    ...
    cp.add_argument("-t", "--types",     default=None, metavar="EXTENSIONS",  help='Extensions e.g. "jpg,png" or "all"')
    cp.add_argument("-f", "--folders",   default=None, metavar="FOLDERS",     help='Folder names e.g. "temp,old" or "all"')
    cp.add_argument("--dry-run",         action="store_true",                 help="Preview without deleting")
    cp.add_argument("-v", "--verbose",   action="store_true",                 help="Show each file/folder name")
    cp.add_argument("--older-than",      type=int, default=None, metavar="DAYS", help="Only files older than N days")
    cp.add_argument("--larger-than",     default=None, metavar="SIZE",        help='Only files larger than SIZE e.g. "10MB"')
    cp.add_argument("--recursive", "-r", action="store_true",                 help="Scan sub-folders recursively")
    ...
    xp = subs.add_parser("config", help="Manage the exclude list")
